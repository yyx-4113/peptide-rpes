...
2. 关联远程并推送（注意本机 git 可能走 `127.0.0.1` 代理；若 `Failed to connect to github.com port 443`，先确认 Clash 等代理已开，或清代理直连）：
   ```bash
   git remote add origin https://github.com/yyx-4113/peptide-rpes.git
   git branch -M main
   git push -u origin main
   ```
3. 打版本 tag 并推送 tag（触发 `.github/workflows/release.yml` 自动建 GitHub Release）：
   ```bash
   git tag v1.0.0
...
GitHub 不适合放 >100 MB 的 `models/pca_latent.npz`。做法：

1. 在 Zenodo 新建 deposit，上传：
   - `data/merged_peptide_library.csv`（451,785 行；可去重后 403,461 行）
   - `models/pca_latent.npz`
   - `models/esm2_embeddings.npy`
2. 在 deposit 元数据里填写作者（Yongxin Yang, ORCID 0009-0004-9698-6552）、许可证（MIT）、与 GitHub 仓库的关系（"is supplemented by" 或 "is documented by"）。
3. 发布后拿到 Zenodo DOI（如 `10.5281/zenodo.XXXXXXX`）。
4. 把该 DOI 回填到 `README.md` 的 "Data availability" 段与 `CITATION.cff` 的 `url`/补充字段，并同步到稿件数据可用性声明。
